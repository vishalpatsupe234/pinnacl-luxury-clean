...
_Environment note (updated 2026-09-24 — supersedes the earlier "no Supabase CLI" note): **the Supabase CLI is available and has been used successfully.** `npx supabase@latest` resolves CLI 2.117.0, and `supabase init` has been run (`supabase/config.toml` exists). **All 22 migrations are applied, and migration history has been repaired and verified — `supabase migration list --db-url …` reports Local = Remote for all 22 versions.** `psql` and Docker remain unavailable, so no local Supabase stack can be started._

_**22, not 23.** `20260922090900_stage1_j10_verification_readonly.sql` is verification-only — pure read-only `SELECT` blocks, not a migration — and was moved to `supabase/verification/` on 2026-09-24 so it can never be applied or repaired into the history table. `supabase/migrations/` holds exactly 22 files._

_The CLI reached the database via `--db-url` rather than `supabase link`; linking is blocked by a CLI 2.117.0 bug on `GET /v1/projects/.../api-keys` (Supabase issue #6445). `--db-url` bypasses the Management API entirely. The project ref is `morweblaakjieerqnopf`; note that `project_id` in `supabase/config.toml` is the **local stack name**, not the ref._

_Migrations in this repository have been applied to the live project by two routes: most were executed manually in the Supabase Dashboard SQL editor, and the migration history table was subsequently backfilled so the CLI's record matches reality. **`supabase migration repair` writes only the history table — it never replays migration SQL**, so the backfill applied no schema change. Four early migrations (`core_tables`, `transaction_and_audit_tables`, `broker_auth_invites`, `lead_management_crm`) use bare `create table` with no `if not exists` and would fail if ever replayed; `db push` should still be treated with care._

_"Implemented" below continues to mean "implemented in application code and passes `npm run build`." Where a claim has been confirmed against the live database, it says so explicitly. `supabase/verification/` holds read-only SQL verification blocks that are deliberately **not** migrations and must never be applied as such._
...
**Current implementation stage:** MVP, pre-production. **Target architecture (approved direction): Supabase is the single source of truth for property data going forward.** **Implemented (F17, 2026-09-19):** this is now the actual state, not just a direction — the homepage's featured properties, `/properties`, `/properties/[slug]`, and `sitemap.ts` all read the Supabase `properties` table (visibility enforced by the `properties_public_read_approved` RLS policy), the same table the Property Listings CMS (`/admin/properties`, `/broker/properties`; see §3) manages, and `/projects` permanently redirects to `/properties`. The legacy `data/properties.json` and its `GET /api/properties` endpoint had zero remaining callers and were removed (see §10). Homepage, `/about`, `/contact`, and `/projects` are genuinely polished; `/properties` and `/properties/[slug]` contain verified live bugs and off-brand styling (§9) — both independent of, and unaffected by, the legacy-vs-target distinction above. The self-governance docs (`PROJECT_CONTEXT.md`, `DECISIONS.md`, `TASKS.md`) are stale since 2026-07-12 and not kept in sync with work completed after that date.

### Architecture Vision — Two Products

**Status is per-product, not section-wide.** Pinnacl Properties (below) is real, running code. **Pinnacl Pro is an Approved Vision with zero code.** Read each product's own label; do not apply "not implemented" to the section as a whole.

**Pinnacl Properties** (this repository, current) — **B2C, Implemented.** The luxury real estate brand described throughout this file: a consumer-facing marketing site and lead-generation funnel serving buyers, sellers, and investors evaluating premium properties. It also carries a **real, working private back office** — invite-only broker/admin authentication, the Property Listings CMS, and the Lead Management CRM (see §3). That back office is internal tooling for this product; it is not, and must not be described as, "Pinnacl Pro."
...
**Pinnacl Pro is an Approved Vision, not an implemented feature.** Nothing describing *Pinnacl Pro* exists in code today: no subscription, no billing, no pricing page, no packaged sellable product.

**This is narrower than it may read, and the distinction matters.** Substantial admin/broker functionality **is** implemented under Pinnacl Properties and must not be described as unbuilt:

| Built and working (Pinnacl Properties) | Still Approved Vision (Pinnacl Pro) |
|---|---|
| Invite-only broker/admin auth + approval gate (§3) | Subscription billing / recurring revenue |
| Property Listings CMS — `/admin/properties`, `/broker/properties` (§3) | AI Brochure generation |
| Lead Management CRM — `/admin/leads`, `/broker/leads` | Broker-facing Landing Pages |
| Supabase RLS, audit logging, Stage 0/Stage 1 schema | A packaged, sellable product |

The overlap in names (`CRM`, `Lead Management`) is the trap: those exist here as **internal tooling for Pinnacl Properties itself**, not as the monetized B2B product. Whether the two ever share code is undecided and not specified anywhere in this file — mark as UNKNOWN rather than assuming either a shared or fully separate codebase.
...
**Stack (Implemented):** Next.js 16.3.5 (App Router, Turbopack), React 19.2.0, TypeScript 5, Tailwind CSS v4.

**Version caveat (verified 2026-09-24):** `package.json` in the working tree specifies `next: ^16.3.5`, and that is what local `npm run build` uses. That change is **not yet committed**, so `package.json`/`package-lock.json` at `HEAD` still pin `^16.1.4` — meaning Vercel currently installs **16.1.4** from the committed lockfile. Both versions build the app successfully. On 16.1.4 the build emits one harmless warning, `Unrecognized key(s) in object: 'agentRules'`, because `next.config.js` sets `agentRules: false`, a key 16.1.4 does not recognise. The dependency bump is queued for its own commit.

**Routing — 7 public page routes + 1 public API route (`POST /api/leads`) + 2 metadata routes** (admin/broker routes are documented under "Authentication" and "Property Listings CMS"):
...
**Data flow (Implemented, single path — F17):** the Server Components `app/page.tsx`, `app/properties/page.tsx`, `app/properties/[slug]/page.tsx` and `app/sitemap.ts` query Supabase directly through `lib/supabase/server.ts`; `PropertiesList.tsx` (client-side) queries Supabase through `lib/supabase/client.ts` whenever filters change, syncing state to the URL via `useSearchParams`. RLS, not the client, decides what is visible. There is no internal HTTP hop and no JSON file left in the property data path.
...
**Public API route (real, not a stub):**
- `POST /api/leads` — authenticates to Google via `googleapis`, initializes a header row in the target Sheet if absent, appends the lead row, sends an HTML owner-notification email via `resend`, and records the enquiry in the Supabase `leads` table.

  **Credentials (Implemented, corrected 2026-09-24):** requires `GOOGLE_SHEET_ID`, **`GOOGLE_SERVICE_ACCOUNT_JSON`**, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `OWNER_NOTIFICATION_EMAIL` — fails with 500 if any are missing. `GOOGLE_SHEET_RANGE` is optional and defaults to `Sheet1!A:G`. The service-account key is supplied **entirely through the environment**, accepted either as raw JSON or base64; `client_email` and `private_key` are validated before use, and literal `\n` sequences in the key are restored to newlines. No credential value is ever logged — failures report only which stage failed.

  ~~Historical: an earlier implementation read the key file from disk via `GOOGLE_SERVICE_ACCOUNT_KEY_PATH`~~ — **RESOLVED.** That approach could never work on Vercel: the key file is gitignored, so it exists in no deployment, and a serverless filesystem is not where a credential belongs. `GOOGLE_SERVICE_ACCOUNT_KEY_PATH` is referenced nowhere in runtime code.

  **Security hardening (Implemented):** server-side field validation (type/length/format checks on every field, plus a cross-field rule requiring a name and at least one of phone/email — closing the gap that let a bare `{}` create a junk lead); HTML-escaping applied only to values reaching the outbound email, never to what is stored; Google Sheets append uses `RAW` value input (not `USER_ENTERED`) to prevent spreadsheet formula injection.

  **Rate limiter:** 5 requests per 10 minutes via an in-memory module-scope `Map`. Single-warm-instance abuse protection only — **not** globally enforced across Vercel serverless instances. Classified as a Medium architecture limitation. Future hardening candidate: shared Supabase-backed rate limiting and/or a honeypot. No immediate code change.

  **Property attribution (Implemented 2026-09-24):** the property `slug → id` lookup uses the **service-role client**, not the anonymous one, so it survives the withdrawal of anonymous `SELECT` on `public.properties` (see §"Pre-registration posture"). Because the service role bypasses RLS, the query restates that policy's predicate explicitly — `select id`, exact slug, `approval_status = 'approved'`, `deleted_at is null` — and selects only `id`. The lead **insert** still uses the caller's anonymous client under the existing `leads_public_insert` policy. The lookup has its own `try/catch`, so a service-role misconfiguration degrades to an unattributed lead rather than a lost one.

  **Still logged verbosely:** step-by-step console output including the spreadsheet ID and the notification from/to addresses. Identifiers, not credentials — but worth trimming.
...
**Security hardening — AUD-01, profile self-update escalation (RESOLVED, verified live 2026-09-19):** `profiles_update_own` (RLS) lets an authenticated user update their own `profiles` row, and RLS cannot restrict individual columns. The only compensating control, the `profiles_role_change_admin_only` trigger, originally guarded `role` alone — so a user could have set their own `status` to `active` (bypassing the manual approval gate above) or reversed a `suspended`/`rejected` decision on themselves, using only their own session and the public anon key. Fixed by extending the existing SECURITY DEFINER function `enforce_profile_role_change_admin_only()` (same function and trigger name, `SET search_path = public` preserved) so a non-`super_admin` caller cannot change `role`, `status`, or `kyc_status`; a `super_admin` can change all three, and self-edits touching none of them (e.g. `full_name`, `phone`) are unaffected. Migration `supabase/migrations/20260919090000_profiles_status_kyc_admin_only.sql`, applied by the owner in the Supabase SQL Editor and confirmed live (`protects_role`, `protects_status`, `protects_kyc_status`, `security_definer`, `search_path_public` all true; trigger still enabled). No production profile rows were modified during verification. **Operational consequence:** `status`/`kyc_status` can no longer be edited by hand in the SQL Editor or Table Editor (no `auth.uid()` there) — use `/admin/brokers`.
...
A real Supabase-backed property management system now exists, layered on top of the `properties`/`builders` tables from the original Curated Broker Network migration. **The public marketing site reads this same `properties` table** — the CMS is the write side, and the public pages (`/`, `/properties`, `/properties/[slug]`, `sitemap.ts`) are its read side, limited for anonymous visitors by the `properties_public_read_approved` RLS policy to approved, non-soft-deleted rows (see §1). The legacy `data/properties.json` no longer exists (F17).

**Schema (`supabase/migrations/20260818100000_property_listings_module.sql`):** extends the existing `properties` table (doesn't duplicate it) with `property_type`, `bedrooms`, `bathrooms`, `area_sqft`. Adds indexes on `city`, `project_status`, `property_type`. Adds a public-read `property-images` Supabase Storage bucket, writes restricted to `super_admin`.

_Historical — superseded by Stage 0 (2026-09-21):_ this migration also added `properties_broker_read_all`, letting any active broker/sales_partner `SELECT` **every** property row — including `pending_review`, `rejected` and soft-deleted ones — because it carried no `approval_status` or `deleted_at` condition. Since RLS policies are permissive and OR'd, that single policy widened every broker's view of the whole table. **It was dropped by `20260921090000_broker_property_scope_stage0.sql` and replaced with `properties_broker_read_approved`.** Do not describe `properties_broker_read_all` as live.

**Current RLS on `public.properties` (four policies, all `to authenticated` — verified live in the Supabase SQL Editor, 2026-09-24):**

| Policy | CMD | Predicate |
|---|---|---|
| `properties_public_read_approved` | SELECT | `approval_status = 'approved' and deleted_at is null` |
| `properties_broker_read_approved` | SELECT | `is_active_broker() and approved and not deleted` |
| `properties_broker_read_own` | SELECT | `source_broker_id = auth.uid()` (own records, any status) |
| `properties_admin_full_access` | ALL | `is_super_admin()` |

`properties_public_read_approved` no longer grants `anon` — see the pre-registration posture note. There is **no INSERT, UPDATE or DELETE policy for brokers**, and no DELETE policy for any role.
...
**Broker (`/broker/properties`):** read-only — search + city/type/status filters, responsive card grid, and a dedicated detail page (`/broker/properties/[id]`) with a real image carousel (prev/next, dot indicators, thumbnail strip). The route performs no writes. Access is granted by **`properties_broker_read_approved`** (approved, non-deleted inventory for an active broker), with **`properties_broker_read_own`** additionally exposing a broker's own source records at any status. ~~Relies on `properties_broker_read_all`~~ — **Historical — superseded by Stage 0 (2026-09-21);** that policy no longer exists. Broker read scope is now approval-scoped rather than table-wide, so a broker can no longer see `pending_review`, `rejected` or soft-deleted rows belonging to anyone else.
...
- **`data/`** — no longer exists. `data/properties.json` (3 placeholder listings) was removed with F17, and the two stray file-listing text dumps that remained (`all-files.txt`, `project-file-list.txt`) were removed in a 2026-09-19 hygiene pass. Property data lives in Supabase.
...
**Phase 1 — Retired (F17, 2026-09-19):** `data/properties.json`, a static file with 3 placeholder listings (no schema, no migrations, no concurrent-write safety), was the original "database". It was removed once every public surface read Supabase instead — see Phase 2.
...
### 5. Deal Attribution Workflow (Planned; **RLS-level security hardening Implemented and Verified live** — see below)
...
**The workflow itself (UI, API, admin verification screen) remains entirely unbuilt — zero application code anywhere references the `deals` table (confirmed by repo-wide grep as of 2026-09-18).** What changed is a security fix at the RLS layer only, made in anticipation of this table eventually being wired up: the original `deals_broker_insert` policy allowed a broker to self-attribute as either `source_broker_id` or `closing_broker_id` while naming *any arbitrary other broker* in the remaining field — meaning a broker could have fabricated another broker's involvement in a deal that never happened. This was closed via `supabase/migrations/20260918100000_deals_rls_hardening.sql`:
- `deals_broker_insert` now requires **both** `source_broker_id` and `closing_broker_id` to equal the inserting broker's own `auth.uid()` (a broker can only self-insert a deal where they are both source and closing broker — both columns are `not null` in the schema, so "or null" was never an available option).
- A new `deals_admin_insert` policy was added (previously, **no** INSERT policy existed for admin at all — only `deals_admin_verify`, which is UPDATE-only), so a genuine two-broker deal still has a legitimate creation path once this feature is eventually built.

**Status: Implemented and Verified live** — applied directly in the Supabase SQL Editor by the owner on 2026-09-18 and confirmed via `pg_policies`: `deals_admin_insert` (INSERT, authenticated), `deals_admin_verify` (UPDATE, authenticated), `deals_broker_insert` (INSERT, authenticated, with the tightened `with check`), `deals_read_own_or_admin` (SELECT, authenticated) all present exactly as expected. This is a database-policy-level fix only — it does not mean the Deal Attribution Workflow is implemented; it means the schema is no longer exploitable in a way that would matter the moment it is.
...
**Currently verified, real gaps in the shipped code (Implemented state, not target state — full detail in §9):** `lib/.env.local` is tracked in git. `/api/leads`'s rate limiter is in-memory and single-instance-only (see §3, Medium architecture limitation, not fixed). ~~`/api/leads` has no rate-limiting or input validation; raw user input is interpolated unescaped into an outbound HTML email~~ — **RESOLVED** via a security hardening pass (server-side validation, HTML-escaping, Sheets `RAW` mode). ~~The JSON-LD `dangerouslySetInnerHTML` pattern noted in §3 is low-risk only because no user-submitted content exists yet~~ — **RESOLVED**: `app/properties/[slug]/page.tsx`'s JSON-LD output now escapes `<` before reaching `dangerouslySetInnerHTML`, closing the script-context XSS risk regardless of whether the underlying property text is user/admin-submitted.
...
- ~~`/api/leads` has no rate-limiting, input validation, or sanitization~~ — **RESOLVED.** Server-side validation, HTML-escaping, and Sheets `RAW` mode are now implemented (see §3). The rate limiter itself is implemented but is an in-memory, single-instance-only counter — moved to Medium as an architecture limitation, not a Critical open gap; see §3 for the exact classification.
...
- ~~**New: the public marketing site has not yet been migrated onto the approved Supabase target architecture.**~~ — **RESOLVED (F17, 2026-09-19).** The homepage's featured properties, `/properties`, `/properties/[slug]` and `sitemap.ts` all read the Supabase `properties` table — the same table the Property Listings CMS manages — and `/projects` redirects to `/properties`. The legacy `data/properties.json` and the `GET /api/properties` endpoint that served it had zero remaining callers and were deleted. See §10.
...
- ~~Server-to-self HTTP fetch pattern on `/properties` and `/properties/[slug]` (fragile, adds latency, no fallback if `NEXT_PUBLIC_SITE_URL` is misconfigured).~~ — **RESOLVED (F17):** those pages now query Supabase directly, and the `/api/properties` endpoint they used to call no longer exists.
...
- ~~Currency mojibake in `data/properties.json` on all 3 listings.~~ — **RESOLVED (F17):** moot, the file was removed.
...
- ~~Root-level stray files unrelated to the project: `hello.py`, `hello3.py`, `roo-test.txt`, `tsconfig.tsbuildinfo`.~~ — **RESOLVED (2026-09-19 hygiene pass):** all four removed after confirming no references; `tsconfig.tsbuildinfo` is now in `.gitignore`.
...
| **Production invite-only broker authentication implemented**: real Supabase Auth wired in (`@supabase/supabase-js`, `@supabase/ssr`, `server-only`); `profiles` extended (added `rejected` status) rather than creating a duplicate `broker_profiles` table; new `invites` table with admin-only RLS; `/broker/login`, `/broker/accept-invite`, `/broker/dashboard` (gated placeholder), `/admin/brokers` (approve/reject + invite UI) all built and passing `npm run build`. Properties, deals, and commissions explicitly out of scope, per the task's own instruction | **Implemented and applied live.** ~~code-level — migrations not yet applied to the live Supabase project from this environment~~ — superseded 2026-09-24: all 22 migrations are applied, and migration history was repaired and verified (`supabase migration list --db-url` reports Local = Remote for all 22). See the environment note at the top of this file. | Direct owner instruction, 2026-08-17. Owner supplied live Supabase URL/publishable key directly; the assistant stopped and explicitly asked before using the service-role key, which the owner then added to `.env.local` themselves |
...
| **Property Listings CMS implemented**: extended the existing `properties`/`builders` tables (added `property_type`/`bedrooms`/`bathrooms`/`area_sqft`, indexes on `city`/`project_status`/`property_type`); added the `properties_broker_read_all` RLS policy (additive, no existing policy changed) — _Historical — superseded by Stage 0 (2026-09-21): that policy was dropped and replaced by `properties_broker_read_approved`_ and a public-read/admin-write `property-images` Storage bucket. Built `/admin/properties` (full CRUD, drag-and-drop image upload, Builder dropdown with get-or-create, inline Featured toggle, soft-delete with confirm) and `/broker/properties` + `/broker/properties/[id]` (read-only, search/filter, image carousel). Deliberately kept as a separate system from the public marketing site's `data/properties.json` — no migration of the public site was in scope | **Implemented and applied live.** ~~code-level — migrations not yet applied to the live Supabase project from this environment~~ — superseded 2026-09-24: all 22 migrations are applied, and migration history was repaired and verified (`supabase migration list --db-url` reports Local = Remote for all 22). See the environment note at the top of this file. | Direct owner instruction across three phased sub-tasks, each stopped and verified (`npm run lint` + `npm run build`) before proceeding to the next |
...
| **Phase 2 security hardening — `deals` RLS INSERT policy tightened**: `deals_broker_insert` changed from "self OR other-broker-arbitrary" to "self AND self" (broker can no longer fabricate a second broker's involvement in a deal); new `deals_admin_insert` policy added (none existed before). Migration `20260918100000_deals_rls_hardening.sql`. The underlying Deal Attribution Workflow itself remains **Planned** and entirely unbuilt — this is a database-policy fix only, made ahead of that feature existing, not a sign it now exists | **Implemented and Verified live** — applied by the owner directly in the Supabase SQL Editor and confirmed via a `pg_policies` query showing all 4 expected policies present with the correct `cmd`/`roles` | Direct owner instruction, 2026-09-18. Migration authored by the assistant; the assistant does not have SQL/CLI access to the live database in this environment, so the owner executed and verified it directly, then reported the exact verification output back |
| **AUD-01 — `profiles` self-update privilege escalation closed**: `profiles_update_own` let any authenticated user update their own profile row while the `profiles_role_change_admin_only` trigger protected only `role`, so a user could potentially self-set `status = 'active'` or reverse a `suspended`/`rejected` status. `enforce_profile_role_change_admin_only()` (still SECURITY DEFINER, `search_path = public`) now blocks non-`super_admin` changes to `role`, `status`, and `kyc_status`; `super_admin` (including the existing `/admin/brokers` approve/reject flow) is unaffected. Migration `20260919090000_profiles_status_kyc_admin_only.sql`. Found by an independent audit, not by the original F1–F17 audit | **Implemented and Verified live** — applied by the owner in the Supabase SQL Editor; live check confirmed all three fields protected, SECURITY DEFINER and `search_path = public` intact, trigger still enabled. No production profile rows were modified during verification | Direct owner instruction, 2026-09-19. Migration authored by the assistant; the owner applied and verified it, as with the `deals` RLS change above |
...
- Never present the 3 placeholder listings (Pinnacl Crest, Aurelia, Bayview — still hardcoded in the dead files `lib/properties.ts` and `components/lib/properties.ts`) as real, marketable inventory.
